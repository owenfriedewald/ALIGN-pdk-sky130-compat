...
| `python3 scripts/inspect_spice.py examples/current_mirror_ota/current_mirror_ota.sp examples/five_transistor_ota/five_transistor_ota.sp examples/telescopic_ota/telescopic_ota.sp` | Passed. Identified `five_transistor_ota` as the smallest next regular-model MOS-only analog block. |
| `docker run ... schematic2layout.py ... generated_runs/five_transistor_ota_label_patch` | Passed. Generated five-transistor OTA after runtime patcher fixed top-level label filtering. |
| `docker run ... run_one_circuit_validation.sh ... five_transistor_ota_label_patch_xsubckt_full --no-sanitize-gds --expand-nf-stack --scale-wl-to-um --mos-as-subckt --uppercase-nets` | Passed. Full flow is clean for five-transistor OTA: Magic DRC `Total DRC errors found: 0`; Netgen LVS `Final result: Circuits match uniquely`; matched 300 devices and 208 nets. |
| `docker run ... schematic2layout.py ... generated_runs/current_mirror_ota_label_patch` | Passed. Generated current-mirror OTA after runtime patcher fixed helper-layer and top-level-label stream-out behavior. Router emitted feasibility warnings but produced `CURRENT_MIRROR_OTA_0.python.gds`. |
| `docker run ... run_one_circuit_validation.sh ... current_mirror_ota_label_patch_xsubckt_full --no-sanitize-gds --expand-nf-stack --scale-wl-to-um --mos-as-subckt --uppercase-nets` | Passed through Magic/Netgen execution with useful failure. Magic DRC reported `Total DRC errors found: 0`; Netgen LVS failed with layout 192 devices/106 nets versus schematic 184 devices/102 nets; PFET count was 80 versus 72. |
| `grep -E '^X(177|178|181|184|186|187|189|190)\b' reports/before_after/current_mirror_ota_label_patch_xsubckt_full/extracted/CURRENT_MIRROR_OTA.extracted.spice` | Passed. Listed the 8 unmatched extracted PFET instances connected to PMOS mirror/output nets. |
| `python3 scripts/analyze_mos_array_units.py generated_runs/current_mirror_ota_label_patch/2_primitives/__primitives__.json` | Expected diagnostic failure exit. Flagged `SCM_PMOS_85912433_X1_Y5` and `_X5_Y1` as `fractional_unit_rounding,unequal_nf_group,high_lvs_count_risk`; `risk_count: 2`. |
| `python3 scripts/analyze_mos_array_units.py generated_runs/five_transistor_ota_label_patch/2_primitives/__primitives__.json` | Passed. Clean five-transistor OTA primitives had `risk_count: 0`. |
| `python3 -m py_compile scripts/analyze_mos_array_units.py` | Passed. New grouped-MOS analyzer syntax check clean. |
...
| `docker run ... schematic2layout.py ... generated_runs/telescopic_ota_unit_counts` | Passed. Generated telescopic OTA after grouped-MOS PDK rewrite. Router emitted feasible-path warnings but produced `TELESCOPIC_OTA_0.python.gds`. |
| `python3 scripts/analyze_mos_array_units.py generated_runs/telescopic_ota_unit_counts/2_primitives/__primitives__.json` | Passed. Telescopic OTA generated MOS primitives had `risk_count: 0`. |
| `docker run ... run_one_circuit_validation.sh ... telescopic_ota_unit_counts_xsubckt_full --coerce-lvt-to-rvt --mos-as-subckt --uppercase-nets` | Passed. Full flow is clean for telescopic OTA under RVT compatibility mode: Magic DRC `Total DRC errors found: 0`; Netgen LVS `Final result: Circuits match uniquely`; matched 10 merged devices and 15 nets. |
| `docker run ... inspect_gds_layers.py generated_runs/buffer_direct_main_gds/BUFFER_0.gds` | Passed. Useful negative evidence: direct default GDS before the in-process PDK stream-out patch still contained helper layers `104:0` and `235:5`. |
| `docker run ... schematic2layout.py -p SKY130_PDK ... generated_runs/buffer_direct_runtime_converter` | Passed. Runtime converter wrapper alone generated successfully, but normal `BUFFER_0.gds` still came from native ALIGN stream-out and retained helper layers. |
| `docker run ... schematic2layout.py -p SKY130_PDK ... generated_runs/buffer_direct_filemap_patch` | Passed. File-map source patch alone generated successfully, but normal `BUFFER_0.gds` still retained helper layers because `align.main.generate_pnr` held an old binding. |
| `docker run ... schematic2layout.py -p SKY130_PDK ... generated_runs/buffer_direct_binding_patch` | Passed. Direct unwrapped ALIGN generation succeeded after refreshing `align.main.generate_pnr` from the PDK import hook. |
| `docker run ... inspect_gds_layers.py generated_runs/buffer_direct_binding_patch/BUFFER_0.gds --compat-map SKY130_PDK/openpdks_compat.json` | Passed. Normal default GDS now has top cell `BUFFER` and no helper layers `100:5`, `104:0`, or `235:5`. |
| `docker run ... run_one_circuit_validation.sh --gds generated_runs/buffer_direct_binding_patch/BUFFER_0.gds --layout-top BUFFER ... --no-sanitize-gds --expand-nf-stack --scale-wl-to-um --mos-as-subckt --uppercase-nets` | Passed. Full flow is clean for direct default GDS buffer output: Magic DRC `Total DRC errors found: 0`; Netgen LVS `Final result: Circuits match uniquely`; matched 4 merged devices and 5 nets. |
