...
def valid_cap_terminals():
    return [
        {"layer": "M4", "netName": "PLUS", "netType": "pin", "rect": [10, 20, 90, 80]},
        {"layer": "CapMIMLayer", "netName": None, "netType": "drawing", "rect": [15, 25, 85, 75]},
...
def test_valid_mim_terminal_topology_passes() -> None:
    MODULE.validate_cap_terminal_topology(valid_cap_terminals())


def test_cross_net_m4_overlap_is_rejected() -> None:
    terminals = valid_cap_terminals()
    terminals[-1]["rect"] = [5, 10, 45, 30]
...
def test_disconnected_mim_contact_is_rejected() -> None:
    terminals = valid_cap_terminals()
    terminals[2]["rect"] = [100, 100, 110, 110]
